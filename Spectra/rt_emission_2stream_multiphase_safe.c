...
int RT_Emit_3D(double PHASE)
{
    ...
    *phi_plus_e, *phi_plus_w, *phi_minus_e, *phi_minus_w, **dphi, *theta_lon, *theta_lat, *phi_lon;
    double R, test, a, b, *lat_rad, kappa_n u_plus_e, kappa_nu_plus_w, wavelength_microns,
    ...
    static double pressure_array_for_scattering_data_in_pascals[500];
    for (i = 0; i < 500; i++)
    {
        fscanf(input_file1, "%le", &pressure_array_for_scattering_data_in_pascals[i]);
    }
    ...
    //read file into array
    static double wavelengths_in_microns[500];
    for (i = 0; i < 500; i++)
    {
        fscanf(input_file2, "%le", &wavelengths_in_microns[i]);
    }
    ...
    double input_val=0;
    num_wavelength_points = 500;
    num_pressure_points = 500;
    ...
    for(i=0; i<NLAMBDA; i++)
    {
        ...
        Locate(500, wavelength_array_for_cloud_scattering_data_in_microns, wavelength_microns, &wavelength_index);
                ...
                if(atmos.lon[m]>=90.0-PHASE && atmos.lon[m]<=270.0-PHASE)
                {
                    for(j=0; j<NTAU; j++)
                    {
                        ...
                        if (pressure > 1e-50)
                        {
                            if (j == 0)
                            {
                                //delta_pressure_bar = pressure * 1e-5;
                            ...
                            if(CLOUDS==1)
                            {
                                Locate(500, pressure_array_for_cloud_scattering_data_in_pascals, pressure, &pressure_index);
                                ...
                                aero_kappa_haze = aero_kappa_pre_tau_haze_interp * haze_wav_tau[pressure_index][wavelength_index] * delta_pressure_bar;
